...
<body data-ng-app="app">
	<div data-ng-controller="crearEmpresaRespuestaController as adminCtrl">
		...
		<div class="jumbotron center_text_align" data-ng-if="${codigoRespuesta} === 200">
			<h1 data-ng-if="'${accion}' == 'EmpresaCrear'">Empresa Creada</h1>
			<h1 data-ng-if="'${accion}' == 'AfiliadoImport'">Creación de Afiliados</h1>
			<h1 data-ng-if="'${accion}' == 'CodigoDescuentoImport'">Importar Códigos</h1>
			<p>
				<a class="btn btn-primary btn-lg" ng-click="adminCtrl.changeView('listar_convenio_empresas')" role="button">Listar
					Empresas</a>
			</p>
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th>Nombre</th>
						<th>Email</th>
						<th>C&oacute;digo asignado</th>
					</tr>
				</thead>
				<tbody>
					<tr data-ng-repeat="employee in adminCtrl.tableFallidos">

						<td>{{employee | pos:0 | tipoDocumento}}</td>
						<td>{{employee | pos:1}}</td>
						<td>{{employee | pos:2}}</td>
						<td>{{employee | pos:3}}</td>
						<td>{{employee | pos:4}}</td>
					</tr>
				</tbody>
			</table>
		</div>
		...
		<input type="hidden" id="csvAfiliadosFallidos" name="csvAfiliadosFallidos" value="${csvAfiliadosFallidos}">
		<input type="hidden" id="csvAfiliadosCompletos" name="csvAfiliadosCompletos" value="${csvAfiliadosCompletos}">
	</div>
	<script
